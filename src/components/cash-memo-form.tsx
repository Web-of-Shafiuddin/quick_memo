...
import { Trash2, Plus, Download, Save, Upload, Crown } from 'lucide-react';
...
const CashMemoForm: React.FC<CashMemoFormProps> = ({ memoData, onUpdate }) => {
  ...
  const saveShopProfile = async () => {
    try {
      ...
      formData.append('ownerName', shopProfile.ownerName);
      formData.append('mobile', shopProfile.mobile);
      formData.append('address', shopProfile.address || '');
      formData.append('bkashNumber', shopProfile.bkashNumber || '');
      formData.append('theme', shopProfile.theme);
      formData.append('isPro', shopProfile.isPro.toString());

      const response = await fetch('/api/shop/save', {
        method: 'POST',
        body: formData
      });
      ...
      if (result.success) {
        ...
        localStorage.setItem('shopProfile', JSON.stringify(result.profile));

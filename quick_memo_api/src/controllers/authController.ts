...
export const userLogin = async (req: Request, res: Response) => {
  try {
    ...
    const user = await pool.query(
      'SELECT user_id, name, email, mobile, preferred_currency, password, shop_name, shop_owner_name, shop_mobile, shop_email, shop_address, shop_logo_url, created_at, updated_at FROM "users" WHERE email = $1',
      [email]
    );
